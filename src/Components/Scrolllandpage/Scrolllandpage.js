import "./Scrolllandpage.css";


const Scrolllandpage = () => {

  return (
    <>
      <div className="scrolllandpage mt-4">

        <div className="scrolllandpages ">

          <img src="./images/scroll1.png" alt="" />
          <img src="./images/scroll2.png" alt=""  />
          <img src="./images/scroll3.avif" alt=""  />
          <img src="./images/scroll9.svg" alt=""  />
          <img src="./images/scroll14.avif" alt=""  />
          <img src="./images/scroll18.webp" alt=""  />
          <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/b/b1/Tata_Consultancy_Services_Logo.svg/2560px-Tata_Consultancy_Services_Logo.svg.png" alt=""  />
          <img src="data:image/png;base64," alt="" />
        </div>

      </div>

    </>
  )

}


export default Scrolllandpage;